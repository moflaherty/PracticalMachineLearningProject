...
```
We removed `r nrow(train[naRow == TRUE,])` rows of NA's with `r format(nrow(train), big.mark=",", scientific=FALSE)` rows remaining. No NA's are found in the test data set.  
...
```
The overall accuracy of this model on the cross-validation data set is **99%** with an expected out-of-sample error rate of **0.01%**. Details about this model are shown here:
```{r echo=TRUE, eval=TRUE}
fitRF
